...
const props = defineProps<{
  list: {
    self: { acct: string; branch: string; balance: number; available: number }[]
    designated: { acct: string; bankCode: string }[]
    nonDesignated: { acct: string; bankCode: string; name: string; notify: string }[]
  }
}>()
...
<template>
  <InlinePageOverlay :title="$t('轉入')" @exit="exit">
    <!-- <div class="p-4">
      <p class="mb-6 mt-2 text-2xl font-bold">{{ $t('選擇轉入帳號') }}</p>
